...
export function GapAnalysisPage({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    ...
    const pending = job.analysis.clarifying_questions.filter(q => !q.answered && !locallyAnswered.has(q.id));
    setCurrentQ(pending[0] ?? null);
  }, [job?.analysis, locallyAnswered]);
  ...
  async function handleAnswer(answer: string) {
    ...
    const answeredId = currentQ.id;
    const answeredQuestion = currentQ.question;

    // Advance immediately so the same question is never shown again
    setLocallyAnswered(s => new Set([...s, answeredId]));
    setAnswerCount(c => c + 1);

    setAnswering(true);
    try {
      await submitAnswer(jobId, answeredId, answeredQuestion, answer);
      const updated = await fetchJob(jobId);
      setJob(updated);
    } catch {
      // ignore, keep polling
  ...
  function skipQuestion() {
    if (!currentQ) return;
    setLocallyAnswered(s => new Set([...s, currentQ.id]));
    setAnswerCount(c => c + 1);
  }
    ...
    <div className="min-h-screen bg-sg-surface flex flex-col">
      ...
      {phase === 'clarification' && analysis && (
        <main className="flex-1 flex min-h-0 overflow-hidden">
          ...
          {/* Center: question (top) + gaps (scrollable below) */}
          <section className="flex-1 flex flex-col min-w-0 min-h-0">

            {/* Question dock — pinned at top */}
            <div className="border-b border-slate-200 bg-white px-5 py-4 flex-shrink-0 shadow-sm">
              {currentQ ? (
                <div>
                  ...
                  <QuestionWidget q={currentQ} onAnswer={handleAnswer} disabled={answering} />
                  <div className="mt-3 flex items-center justify-between">
                    <button onClick={skipQuestion} className="text-xs text-slate-400 hover:text-slate-600 transition-colors">
                      Skip this question
                    </button>
                    <button onClick={handleGenerateReport} disabled={analysis.report_readiness < 30}
                      className="flex items-center gap-2 rounded-lg bg-sg px-3 py-1.5 text-xs font-bold text-white disabled:opacity-30 hover:bg-sg-hover transition-colors">
